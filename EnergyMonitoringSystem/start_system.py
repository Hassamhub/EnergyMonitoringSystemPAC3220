...
def start_modbus_poller():
    """Start the Modbus poller service"""
    print("Starting Modbus Poller...")
    try:
        poller_path = Path(__file__).parent.parent / "app.py"
        cmd = [sys.executable, str(poller_path)]

        return subprocess.Popen(cmd)
    except Exception as e:
        print(f"[ERROR] Failed to start Modbus poller: {e}")
        return None
...
def main():
    ...
    try:
        ...
        # Start Modbus poller
        poller_process = start_modbus_poller()
        if poller_process:
            processes.append(("Modbus Poller", poller_process))
